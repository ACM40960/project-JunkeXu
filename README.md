...
We can see the prediction result and probability of each picture in the test file from the csv result. More than 0.5 is a cat, less than 0.5 is a dog. For example, the second picture of test has a high probability of being a dog because the value is much less than 0.5. The probability is not particularly high due to the limitations of the model. If a higher probability change is required, the model run time may reach 10 hours, so use a simple model for the output of the outcome prediction.
